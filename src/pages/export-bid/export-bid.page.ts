...
@Component({
  selector: 'app-export-bid',
  templateUrl: './export-bid.page.html',
  styleUrls: ['./export-bid.page.scss'],
})
export class ExportBidPage {
  ...
  export(destination) {
    if (destination == undefined || destination == null || destination == '') {
      this.ComponentService.presentToast('Please select the destination to export', 'danger');
    }
    else {
      if (this.page_type == '1') {
        ...
        if (destination != 'local') {
          ...
          pdfMake.createPdf(self.pdfmake.docDefinition).getDataUrl(function (dataUrl) {
            self.APIService.sendData('saveBidPdf', { file: dataUrl, file_name: file_name, path: path, type: destination, jobId: self.jobId, tradeId: self.tradeId }).subscribe((result) => {
