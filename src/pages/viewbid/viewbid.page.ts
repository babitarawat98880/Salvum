...
export class ViewbidPage {
  ...
  goToTrade(jobId, tradeId) {
    this.navCtrl.navigateForward('EdittradePage', {
      state: {
        tradeId: tradeId,
        job_id: jobId,
        isEdit: '1',
  ...
  signContract(Iid, Bid, tradeId) {
    this.navCtrl.navigateForward('AddcontractPage', {
      state: {
        InvId: Iid,
        BidId: Bid,
        jobId: this.jobId,
  ...
  replyBid(userId, tradeId, bidId) {
    if (userId == undefined || userId == null || userId == '') {
      ...
      this.navCtrl.navigateForward('compose', {
        state: {
          userId: userId,
          tradeId: tradeId,
          jobId: this.jobId,
  ...
  sendReply(bidId, reply_comments, index, posted_email, name, Iid) {
    if (reply_comments != '' && reply_comments != null && reply_comments != undefined) {
      ...
      this.APIService.sendData('replyComment', {
        'bidId': bidId, 'bid_comment': reply_comments, 'email': posted_email, 'name': name, 'Iid': Iid, 'baseUrl': baseUrl
      }).subscribe((result) => {
