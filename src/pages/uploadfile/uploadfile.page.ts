...
export class UploadfilePage {
  ...
  userId: any = localStorage.getItem('userinfo');
                ...
                this.uploader.onCompleteItem = (item:any, response:any, status:any, headers:any) => {
                  ...
                  if(response != '')
                  {
                    if(response[0].status == '1')
                    {
                     ...
                     {
                      this.componentService.presentToast('Files uploaded successfully.','success');
                      this.dismiss(null);
                     }
                    }
                    else
                    {
                     this.componentService.presentToast( 'Error while uploading.','danger');
                  ...
                  this.uploader.onCompleteItem = (item:any, response:any, status:any, headers:any) => {
                    ...
                    if(response != '')
                    {
                      if(response[0].status == '1')
                      {
                        ...
                        if(this.uploader.getNotUploadedItems().length == 0){
                          this.modalController.dismiss(all_files);
                        }
                      }
                      else
                      {
   ...
   dismiss(tradeId:any){
      if(this.navParams.get('bid_upload') == '1')
        {
          this.modalController.dismiss(this.uploader.queue);
        }
        else
        {
          if(this.navParams.get('page_type') == '1')
